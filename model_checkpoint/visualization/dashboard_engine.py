...
class DashboardEngine:
    ...
    def create_dashboard_from_template(
        self, template_name: str, dashboard_id: str, title: Optional[str] = None
    ...
    def generate_chart_data(
        ...
        if (
            not force_refresh
            and cache_key in self._chart_cache
            and current_time - self._cache_timestamps.get(cache_key, 0)
            < chart_config.cache_duration
        ):
        ...
        try:
            ...
            for metric_name, trend_analysis in trends.items():
                if self._apply_filters(
                    trend_analysis.__dict__, chart_config.filter_criteria
                ):
                    data_point = {
                        "metric_name": metric_name,
    ...
    def _process_chart_data(
        ...
        for item in raw_data:
            ...
            data_point = {"x": x_value, "y": y_value}
    ...
    def get_dashboard_html(
        self, dashboard_id: str, experiment_id: Optional[str] = None
    ) -> Optional[str]:
        """
