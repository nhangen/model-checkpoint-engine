...
class CheckpointVerifier:
    ...
    def __init__(
        self,
        database_connection: EnhancedDatabaseConnection,
    ...
    def verify_checkpoint(
        self, checkpoint_id: str, repair_on_failure: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not checkpoint.file_path or not os.path.exists(checkpoint.file_path):
                ...
                result["errors"].append(
                    f"Checkpoint file not found: {checkpoint.file_path}"
                )
            ...
            if checkpoint.checksum:
                verification = self.checksum_calculator.verify_with_metadata(
                    checkpoint.file_path, checkpoint.checksum, checkpoint.file_size
                )
    ...
    def verify_experiment_checkpoints(
        self, experiment_id: str, repair_on_failure: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        for checkpoint in checkpoints:
            checkpoint_result = self.verify_checkpoint(checkpoint.id, repair_on_failure)
        ...
        results["summary"] = {
            "integrity_percentage": (
                (results["verified"] / results["total_checkpoints"] * 100)
                if results["total_checkpoints"] > 0
                else 0
            ),
            "repair_success_rate": (
                (
                    results["repaired"]
                    / (results["repaired"] + results["irreparable"])
                    * 100
                )
                if (results["repaired"] + results["irreparable"]) > 0
                else 0
            ),
        }
            ...
            if os.path.exists(backup_path):
                ...
                try:
                    ...
                    recovery_result["details"].append(
                        f"Backup {backup_path} corrupted: {e}"
                    )

        # Strategy 2: Look for similar checkpoints in the same experiment
        similar_checkpoints = self.db.get_checkpoints_by_experiment(
            checkpoint.experiment_id
        )
        for similar_checkpoint in similar_checkpoints:
            if (
                similar_checkpoint.id != checkpoint.id
                and similar_checkpoint.epoch == checkpoint.epoch
                and os.path.exists(similar_checkpoint.file_path)
            ):
                ...
                recovery_result.update(
                    {
                        ...
                        "details": [
                            f"Using similar checkpoint: {similar_checkpoint.id}"
                        ],
                    }
                )
                return recovery_result
    ...
    def _attempt_corruption_repair(self, checkpoint) -> Dict[str, Any]:
        ...
        """
        repair_result = {"success": False, "method": None, "details": []}
    ...
    def create_checkpoint_backup(
        self, checkpoint_id: str, backup_dir: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        checkpoint = self.db.get_checkpoint(checkpoint_id)
        if not checkpoint:
            return {"success": False, "error": "Checkpoint not found in database"}
        ...
        try:
            ...
            if self._test_checkpoint_loadability(backup_path):
                ...
                backup_checksum = self.checksum_calculator.calculate_file_checksum(
                    backup_path
                )
