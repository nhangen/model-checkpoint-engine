...
class HTMLReportGenerator:
    ...
    def generate_training_report(self, output_dir: str = '.', format_type: str = 'html') -> str:
        """
    ...
    def _generate_html_content(self, experiment, metrics) -> str:
        ...
        for metric_name, metric_list in metrics_by_name.items():
            ...
            metrics_summary += f"<li><strong>{metric_name}:</strong> {latest_value:.4f}</li>"
        
        # Generate HTML
        # Create configuration items
        config_items = '\n'.join([f'<li><strong>{k}:</strong> {v}</li>' for k, v in experiment.config.items()])
