...
class TestEnhancedCheckpointManager:
    ...
    def test_load_checkpoint_by_type(self, manager, model_and_optimizer):
        ...
        checkpoint_id_2 = manager.save_checkpoint(model=model, epoch=2, loss=0.5)  # Best
        checkpoint_id_3 = manager.save_checkpoint(model=model, epoch=3, loss=0.8)  # Latest
    ...
    def test_migration_integration(self, db_path):
        ...
        with db._get_connection() as conn:
            ...
            for field in enhanced_fields:
                assert field in columns, f"Enhanced field '{field}' not found in checkpoints table"